...
def lala(year):
    ...
    return train_rf_rh98(year, sierras_box, True, True)
...
def train_rf_rh98(year, geometry, save: bool = True, log: bool = False):
    # Get data for this year.
    logger.debug("Load training data from a pickle file.")
    gedi = load_pickle(f"{DATA_PATH}/rf/rh98/unburned/gedi_match_{year}.pkl")
    ...
    landsat_columns = [f"{x}_{y}"
                       for y in range(year - landsat_timeseries_legth, year)
                       for x in gedi_raster_matching.get_landsat_bands(y)]
    columns_to_use = ['rh_98', 'elevation_median', 'slope_median',
                      'aspect_median', 'soil_median', 'gedi_year', 'gedi_month'] + landsat_columns
    ...
    if log:
        logger.debug("Optimizing for log")
        gedi_prepared = gedi_prepared[gedi_prepared.rh_98 != 0]
        gedi_prepared[dep_var] = np.log(gedi_prepared[dep_var])
    ...
    if save:
        logger.debug(f"Save RF model for inference.")
        if log:
            save_pickle(f"{DATA_PATH}/rf/rh98/models_log/model_{year}.pkl", m)
            logger.debug(f"Save TabularPandas object for inference.")
            to.export(f"{DATA_PATH}/rf/rh98/models_log/to_{year}.pkl")
        else:
            save_pickle(f"{DATA_PATH}/rf/rh98/models/model_{year}.pkl", m)
            logger.debug(f"Save TabularPandas object for inference.")
            to.export(f"{DATA_PATH}/rf/rh98/models/to_{year}.pkl")
